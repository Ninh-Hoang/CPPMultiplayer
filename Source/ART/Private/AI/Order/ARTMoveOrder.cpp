...
#include "AI/Order/ARTMoveOrder.h"
#include "Ability/ARTGlobalTags.h"

UARTMoveOrder::UARTMoveOrder()
{
	TargetType = EARTTargetType::LOCATION;
	bIsCreatingIndividualTargetLocations = true;

	//TagRequirements.SourceBlockedTags.AddTag(UARTGlobalTags::Status_Changing_Immobilized());
	//TagRequirements.SourceBlockedTags.AddTag(UARTGlobalTags::Status_Changing_Constructing());
}

void UARTMoveOrder::CreateIndividualTargetLocations(const TArray<AActor*>& OrderedActors,
...
void UARTMoveOrder::CalculateFormation(int32 UnitCount, const FVector2D Direction, const FVector2D TargetLocation,
...
FVector2D UARTMoveOrder::GetCenterOfGroup(const TArray<AActor*>& Actors) const
{
...
int32 UARTMoveOrder::GetFormationRank(const AActor* Unit) const
{
	if (!IsValid(Unit))
	{
		return 0;
	...
	//const UARTFormationRankComponent* FormationRankComponent = Unit->FindComponentByClass<UARTFormationRankComponent>();
