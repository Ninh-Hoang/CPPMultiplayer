...
void ABaseCharacter::LookAtCursor() {
	...
	if (DebugAimDrawing > 0 && IsLocallyControlled()) {
		if (CurrentWeapon) {
			FVector WeaponOffSet = CurrentWeapon->GetActorLocation() - GetActorLocation();

			DrawDebugSphere(GetWorld(), Intersection+ WeaponOffSet , 10, 12, FColor::Blue, false, GetWorld()->GetDeltaSeconds(), 0, 1);
