...
FItemAddResult UInventoryComponent::TryAddItem_Internal(UItem* Item) {
	if (GetOwner() && GetOwner()->HasAuthority() && Item) {
		...
		if (Items.Num() + 1 > GetCapacity()) {
			return FItemAddResult::AddedNone(AddAmount, LOCTEXT("InventoryCapacityFullText", "Could not add item. Inventory is full."));
		}
		...
		if (FMath::IsNearlyZero(Item->Weight)) {
			...
			if (GetCurrentWeight() + Item->Weight > GetWeightCapacity()) {
				return FItemAddResult::AddedNone(AddAmount, LOCTEXT("InventoryTooMuchWeight", "Could not add item. Exceed inventory weight capacity"));
			}
		}
		...
		if (Item->bStackable) {
			...
			if (UItem* ExistingItem = FindItem(Item)) {
				...
				if (ExistingItem->GetQuantity() < ExistingItem->MaxStackSize) {
					...
					return FItemAddResult::AddedNone(AddAmount, FText::Format(LOCTEXT("InventoryFullStackText", "Could not add {ItemName}. Already max stack."), Item->ItemDisplayName));
				}
			}
			else {
				AddItem(Item);
